        ...
        if (voxel_label == VoxelLabel::INTERSECTED) {
          ...
          if (voxel_label == VoxelLabel::INTERSECTED) {
            ...
            if (semantics != Semantics::UNKOWN) {
              ...
              json sem_json = {{"type", sem_str}};

              auto found =
                  find(semantics_surfaces[city_object_key].begin(),
                       semantics_surfaces[city_object_key].end(), sem_json);
              if (found == semantics_surfaces[city_object_key].end()) {
                semantics_surfaces[city_object_key].push_back(sem_json);
                semantics_values[city_object_key].push_back(
              ...
              semantics_values[city_object_key].push_back(nullptr);
            }
