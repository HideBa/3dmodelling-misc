...
// This function finds the ground surface of the building. The ground surface is the surface with the lowest average height.
vec<vec<int>> find_lowest_surface(const vec<vec<vec<int>>> &shell, const vec<vec<int>> &cj_vertices,
                                  const vec<double> &cj_scales, const vec<double> &cj_translates) {
...
void removeArrayFromArray(vec<vec<vec<int>>> &array1, const vec<vec<int>> &array2) {
    auto it = find_if(array1.begin(), array1.end(),
                      [&array2](const vec<vec<int>> &element) {
...
vec<vec<vec<int>>> find_roof_surfaces(const vec<vec<vec<int>>> &shell, const vec<vec<int>> &cj_vertices,
                                      const vec<double> &cj_scales, const vec<double> &cj_translates,
...
double calc_roof_height(const vec<vec<vec<int>>> &roof_surfaces, const vec<vec<int>> &cj_vertices,
                        const vec<double> &cj_scales, const vec<double> &cj_translates) {
    pair<int, double> heigher_surface;
    for (size_t i = 0; i < roof_surfaces.size(); i++) {
        ...
        for (size_t j = 0; j < surface.size(); j++) {
            ...
            bool is_vertical = is_vertical_surface(points);
            if (!is_vertical) {
                if (heigher_surface.second == 0) {
                    heigher_surface = {i, average_height};
                }
                if (average_height > heigher_surface.second) {
                    heigher_surface = {i, average_height};
                }
            }
        }
    }
    return heigher_surface.second;
}


json lod0_2(json &js) {
    vec<vec<int>> cj_vertices = js["vertices"];
    vec<double> cj_translates = js["transform"]["translate"];
    vec<double> cj_scales = js["transform"]["scale"];
            ...
            for (auto &g: co.value()["geometry"]) {
                if (g["lod"] == "2.2" || "2") {
                    for (auto &shell: g["boundaries"]) {
                        ...
                        vec<vec<int>> ground_surface = find_lowest_surface(casted_shell, cj_vertices, cj_scales,
                                                                           cj_translates);
            ...
            for (auto &g: co.value()["geometry"]) {
                if (g["lod"] == "2.2" || "2") {
                    for (auto &shell: g["boundaries"]) {
                        ...
                        vec<vec<vec<int>>> lod0_2_boundaries;
                        pair<int, double> lower_surface;
                        vec<vec<int>> ground_surface = find_lowest_surface(shell.get<vec<vec<vec<int>>> >(),
                                                                           js["vertices"], scales, translates);
                        lod0_2_boundaries.push_back(ground_surface);


                        // Extract roof surfaces
                        vec<vec<vec<int>>> roof_surfaces = find_roof_surfaces(shell.get<vec<vec<vec<int>>> >(),
                                                                              js["vertices"], scales, translates,
                                                                              ground_surface);

                        // Calculate roof height
                        double roof_height = calc_roof_height(roof_surfaces, js["vertices"], scales, translates);
